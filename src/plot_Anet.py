...
def main(chamber_volume, window_size, plot_window, pressure_pa,
    ...
    leaf_area_cm2 = [leaf_area_cm2_init]  # Mutable for GUI updates
    stop_requested = [False]  # Mutable flag for button callback
    ...
    def update_leaf_area(text):
        try:
            ...
            print("Invalid leaf area input. Enter a positive number.")

    def stop_callback(event):
        print("Stop Logging button pressed.")
    ...
    # Stop button
    ax_button = plt.axes([0.45, 0.02, 0.15, 0.05])
    stop_button = Button(ax_button, "Stop Logging")
    ...
    try:
        while not stop_requested[0]:
            ...
            if sensor.read_measurement():
                ...
                if len(co2_window) >= 3:
                    ...
                    mol_flux = ppm_to_umol_s(corrected_slope, chamber_volume, temp_K, pressure_pa)
                    mol_flux_upper = ppm_to_umol_s(slope_upper, chamber_volume, temp_K, pressure_pa)
                    mol_flux_lower = ppm_to_umol_s(slope_lower, chamber_volume, temp_K, pressure_pa)
                    ...
                    ci_fill = ax.fill_between(times_rel,
                                              ...
                                              label="95% CI" if not ci_filled_once else None)
        ...
        plt.ioff()
        plt.show()
        print("Logging stopped. Plot closed.")
