...
class PhotosynthesisLogger:
    def __init__(self, chamber_volume, window_size, plot_window,
        ...
        self.temp_values = deque()
        self.rh_values = deque()
    ...
    def start_logging(self, event):
        ...
        print(f"\nLogging started. Leaf area = {self.leaf_area_cm2:.1f} cm²")
        self.status_text.set_text(
            f"Status: Logging... (Leaf = {self.leaf_area_cm2:.1f} cm²)")
        plt.draw()
            ...
            while not self.stop_requested:
                ...
                if self.zero_run_started:
                    try:
                        ...
                        print(f"Sensor error during zero run: {e}")
                        time.sleep(1)
                        continue
                ...
                if self.logging_started:
                    ...
                    try:
                        ...
                        print(f"Sensor error during logging: {e}")
                        time.sleep(1)
                        continue
                    ...
                    while len(self.temp_values) > self.window_size:
                        self.temp_values.popleft()
                    while len(self.rh_values) > self.window_size:
                        self.rh_values.popleft()
                    ...
                    if len(self.co2_window) >= 3:
                        ...
                        temp_K = temp + 273.15
                        leaf_area_m2 = self.leaf_area_cm2 / 10000.0

                        flux = self.ppm_to_umol_s(corr_slope, self.chamber_volume,
                        ...
                        self.ci_fill = self.ax.fill_between(
                            times_rel, list(self.anet_lower), list(self.anet_upper),
                            color='seagreen', alpha=0.3)

                        temp_times_rel = times_rel[-len(self.temp_values):]
                        self.temp_line.set_xdata(temp_times_rel)
                        self.temp_line.set_ydata(list(self.temp_values))

                        self.rh_line.set_xdata(temp_times_rel)
                        self.rh_line.set_ydata(list(self.rh_values))
...
if __name__ == "__main__":
    ...
    logger = PhotosynthesisLogger(chamber_volume=1.2,
                                 window_size=6,
                                 plot_window=300,
                                 pressure_pa=101325,
